...
export interface HyperDecodeServiceConfig {
    serviceName: string;
    ...
    osrmTaskDefinitionArn: pulumi.Input<string>;
}

// Application configuration
const hyperDecodeReleaseConfig = new pulumi.Config("hyper-decode");
const releaseTag = hyperDecodeReleaseConfig.require("releaseTag");
...
function createRepoAndImage(serviceName: string) {
    ...
    const image = new awsx.ecr.Image(serviceName, {
        ...
        context: "../../../hyperdecode/backend", // Path to your application directory
        imageTag: releaseTag,
...
export function createHyperDecodeService(config: HyperDecodeServiceConfig) {
    const { serviceName, containerName, containerPort, ecsClusterName, ecsClusterArn, subnetIds, securityGroupId, loadBalancerArn, certificateArn, httpsListenerArn, appFqdn, fargateSecurityGroupId, osrmTaskDefinitionArn } = config;
    ...
    const taskDefinition = pulumi.output(fargateSecurityGroupId).apply((sgId) => {
        const taskDefinition = pulumi.output(osrmTaskDefinitionArn).apply(osrmWrapperArn => {
                            ...
                            environment: [
                                {
                                    ...
                                    value: `https://${appFqdn}/hyper-decode-api`,
                                },
                                {
    ...
    const targetGroup = new aws.lb.TargetGroup(serviceName, {
        ...
        healthCheck: {
            ...
            interval: 30,
            matcher: "200-299",
            path: "/hyper-decode-api/p/health",
            protocol: "HTTP",
            timeout: 5,
    ...
    const httpsListenerRule = new aws.lb.ListenerRule(`${serviceName}-listener-rule`, {
        ...
        priority: 200, // Higher priority number than fleet-mgmt (which uses 100)
        actions: [{
            type: "forward",
        ...
        conditions: [{
            hostHeader: {
                values: [`hyper-decode.amaan.click`], // hyper-decode uses hyper-decode subdomain
            },
        }],
    });
